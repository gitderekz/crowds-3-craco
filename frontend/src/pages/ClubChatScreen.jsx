...
import { FaPaperPlane, FaSmile, FaFile, FaUserFriends, FaInfoCircle, FaCheck, FaSpinner, FaPhone, FaVideo, FaBattleNet } from 'react-icons/fa';
...
const ClubChatScreen = ({ room, onClose, onOpenPrivateChat, setIsAuthModalOpen }) => {
  ...
  // Generate club room ID
  const clubRoomId = String(room.photoId);
    ...
    const fetchParticipantsAndRoom = async () => {
      ...
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/rooms/${clubRoomId}/${room.name}/participants?isGroup=true`,
          {
            headers: {
  ...
  }, [clubRoomId, room.name, token]);

  // Fetch initial presence data & all admirers
  useEffect(() => {
    const fetchInitialPresence = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_API_URL}/rooms/${clubRoomId}/presence`,
          {
    ...
    const fetchAllAdmirers = async () => {
      try {
        const admirersResponse = await api.get(`/mingle/all-admirers/${currentUserId}/${clubRoomId}`);
        setAllAdmirers(admirersResponse.data.allAdmirers || []);
        setIsMingling(admirersResponse.data.isMingling || false);
      } catch (error) {
    ...
    fetchAllAdmirers();
  }, [clubRoomId, currentUserId, token]);
  ...
  useEffect(() => {
    const loadMessages = async () => {
      setIsLoading(true);
      try {
        const response = await api.get(`/messages/${clubRoomId}`);
  ...
  useEffect(() => {
    ...
    socketRef.current.on('connect', () => {
      console.log('Club chat socket connected:', socketRef.current.id);
    ...
    // Mingle socket events
    socketRef.current.on('mingle-match', ({ matchedUserId, matchedUserName }) => {
      showAlert(`You've matched with ${matchedUserName}!`, 'success');
      setMatches(prev => [...prev, { id: matchedUserId, username: matchedUserName }]);
    });
    
    socketRef.current.on('mingle-chosen', ({ chooserId }) => {
      // Handle when someone chooses you
    });
    
    socketRef.current.on('mingle-ignored', ({ chooserId }) => {
      // Handle when someone ignores you
    });
    ...
    const handleNewMessage = (message) => {
      console.log('ClubChat received message:', message.tempId, tempMessagesRef.current);
    ...
    // Typing handler
    const handleTypingEvent = ({ userId, isTyping: typing }) => {
      setTypingUsers(prev => {
        if (typing) {
    ...
    const handlePresenceUpdated = ({ presentCount, userId, isPresent: userIsPresent }) => {
    ...
    socketRef.current.on('presence-updated', handlePresenceUpdated);
    ...
    return () => {
      if (socketRef.current) {
        ...
        socketRef.current.off('presence-updated', handlePresenceUpdated);
        socketRef.current.disconnect();
      }
    };
  }, [clubRoomId, currentUserId]);
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      if (mediaFiles.length) {
        ...
        const uploadResponse = await uploadMedia(clubRoomId, type);
        if (uploadResponse) {
  ...
  const typingText = typingUsers.length === 1 ? 
    `${participants.find(u => u.id === typingUsers[0])?.username || 'Someone'} is typing...` :
  ...
  const toggleMingle = async () => {
    try {
      const response = await api.post(`/mingle/toggle/${clubRoomId}`, { userId: currentUserId });
  ...
  const openMingle = async () => {
    if (isMingling) {
      try {
        const matchesResponse = await api.get(`/mingle/potential-matches/${currentUserId}/${clubRoomId}`);
        setPotentialMatches(matchesResponse.data);
        
        const admiredResponse = await api.get(`/mingle/admired/${currentUserId}/${clubRoomId}`);
        setAdmired(admiredResponse.data);
        
        const admirersResponse = await api.get(`/mingle/admirers/${currentUserId}/${clubRoomId}`);
        setAdmirers(admirersResponse.data);
        
        const existingMatchesResponse = await api.get(`/mingle/matches/${currentUserId}/${clubRoomId}`);
        setMatches(existingMatchesResponse.data);
        
        setShowMingleModal(true);
      } catch (error) {
        console.error('Error fetching mingle data:', error);
      }
    }
  };
  ...
  }, [socket, clubRoomId, currentUserId, setIncomingCall]);
  ...
  return (
    <div className={`club-chat-container ${theme}`}>
      ...
      <div className={`chat-header ${theme}`}>
        <h3>{room.name?.toUpperCase() || 'CLUB'} CHAT</h3>
        <div className={`typing-indicator ${typingUsers.length > 0 ? '' : 'active'} ${theme}`}>
          {typingText}
        </div>
        <div className="call-buttons">
          <button className={`call-btn ${theme}`} onClick={() => startGroupCall('video')}>
            <FaVideo />
          </button>
          <button className={`call-btn ${theme}`} onClick={() => startGroupCall('audio')}>
            <FaPhone />
          </button>
      ...
      <div className={`mobile-tabs ${theme}`}>
        <button className={`tab-button ${activeTab === 'participants' ? 'active' : ''}`} onClick={() => setActiveTab('participants')}>
        ...
        <div className={`participants-section ${theme} ${activeTab === 'participants' ? 'mobile-active' : ''}`}>
          ...
          {loadingParticipants ? (
            ...
            <div className="participants-list">
              {participants.filter((u) => u.id !== currentUserId).map(user => (
                <div key={user.id} className="participant-card" onClick={() => onOpenPrivateChat(user)}>
                  ...
                  <div className="participant-info">
                    ...
                    <span className="participant-status">{user.online ? 'Online' : 'Offline'}</span>
                  </div>
        ...
        <div className={`chat-section ${theme} ${activeTab === 'chat' ? 'mobile-active' : ''}`}>
            ...
            <div className={`mingle-container ${theme}`}>
              <div className={`vertical-toggle ${isMingling ? 'active' : ''}`} onClick={toggleMingle}>
                <div className="toggle-track">
                  <div className="toggle-thumb">{isMingling ? '🌑' : '🌕'}</div>
                </div>
              </div>
              <button className={`attendance-btn mingle ${theme} ${isMingling ? 'active' : ''}`} onClick={openMingle} disabled={!isMingling}>
                {isMingling ? <><span role="img" aria-label="Mingle">👥</span> <span className="admirer">{allAdmirers.length}</span></> : 'disabled'}
              </button>
            </div>

            <button className={`attendance-btn present ${theme} ${isPresent ? 'active' : ''}`} onClick={handlePresentClick} disabled={loadingPresence || isPresent}>
              <span role="img" aria-label="Present">✌️</span> Going <span className="count">({attendeesCount})</span>
            </button>

            <button className={`attendance-btn absent ${theme} ${isPresent ? '' : 'active'}`} onClick={handleAbsentClick} disabled={loadingPresence || !isPresent}>
              <span role="img" aria-label="Absent">🙇‍♂️</span> Absent
            </button>
          </div>
          ...
          <div className="messages-container">
            {isLoading ? (
              ...
              messages.map(msg => (
                <div key={msg.id || msg.tempId} className={`message ${parseInt(msg.senderId) === parseInt(currentUserId) ? 'sent' : 'received'} ${theme} ${msg.isPending ? 'pending' : ''}`}>
          ...
          {mediaFiles.length > 0 && (
            <MediaPreview mediaFiles={mediaFiles} onRemove={(index) => setMediaFiles(prev => prev.filter((_, i) => i !== index))} uploadProgress={uploadProgress} />
          )}
          ...
          <div className="message-input">
            <button className={`emoji-btn ${theme}`} onClick={() => setShowEmojiPicker(!showEmojiPicker)}>
              <FaSmile />
            </button>
            {showEmojiPicker && (
              <div className="emoji-picker">
                <EmojiPicker onEmojiClick={(e) => { setMessage(m => m + e.emoji); setShowEmojiPicker(false); }} />
              </div>
            )}
            ...
            <input
              type="text"
              value={message}
              onChange={(e) => { setMessage(e.target.value); handleTyping(!!e.target.value); }}
              ...
              className={theme}
            />
            <button onClick={handleSendMessage} className={`send-button ${theme}`} disabled={isSending || isUploading}>
        ...
        <div className={`info-section ${theme} ${activeTab === 'info' ? 'mobile-active' : ''}`}>
          ...
          <div className="info-card">
            ...
            <div className="info-item">
              ...
              <span className="info-value">{room.location ? JSON.parse(room.location)?.name || 'Unknown' : 'Unknown'}</span>
            </div>
      ...
      {/* Mingle modal - Keep your existing mingle modal JSX */}
      {showMingleModal && (
        <div className={`mingle-modal-overlay ${theme}`}>
          {/* Your existing mingle modal content */}
        </div>
      )}
...
//     // mingle socket connection
//     socketRef.current.on('mingle-match', ({ matchedUserId, matchedUserName }) => {
//       showAlert(`You've matched with ${matchedUserName}!`, 'success');
//       setMatches(prev => [...prev, matchedUserId]);
//     });
    
//     socketRef.current.on('mingle-chosen', ({ chooserId }) => {
//       setAdmirers(prev => [...prev, chooserId]);
//     });
    
//     socketRef.current.on('mingle-ignored', ({ chooserId }) => {
//       setAdmirers(prev => prev.filter(id => id !== chooserId));
//     });
...
//     try {
//       const response = await api.post('/mingle/choose', {
...
//         `You've chosen ${user.username}`, 
//       response.data.isMatch ? 'success' : 'info');
//     } catch (error) {
//       console.error('Error choosing match:', error);
//       showAlert('Failed to choose match', 'error');
//     }
//   };
  
//   const ignoreMatch = async (user) => {
//     try {
//       await api.post('/mingle/ignore', {
//         chooserId: currentUserId,
//         chosenId: user.id
//       },
//       {
...
//       showAlert('Failed to ignore match', 'error');
//     };
//   };
...
//       {showMingleModal && (
//         <div className={`mingle-modal-overlay ${theme}`}>
//           <div className={`mingle-modal ${theme}`}>
//             <div className="mingle-header">
//               <h3>Find Someone to Mingle With Today</h3>
//               <button onClick={() => setShowMingleModal(false)}>✕</button>
//             </div>
...
//                       {selectedMatch.bio || 'No bio available'}
//                     </div>
                    
//                     <div className="mingle-actions">
//                       <button 
//                         className={`mingle-action-btn choose ${theme}`}
//                         onClick={(e) => {
//                           e.stopPropagation();
...
//                   </>
//                 ) : (
//                   <div className="mingle-empty-state">
