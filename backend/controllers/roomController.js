...
exports.createRoom = async (req, res) => {
  ...
  try {
    const { userIds, isGroup, name, roomId } = req.body;
    ...
    if (!isGroup && userIds.length === 1) {
      const [user1, user2] = [creatorId, userIds[0]].sort((a, b) => a - b);
      ...
      await db.roomuser.bulkCreate([
        { roomId: privateRoomId, userId: user1, createdAt: new Date(), updatedAt: new Date() },
        { roomId: privateRoomId, userId: user2, createdAt: new Date(), updatedAt: new Date() }
      ]);
    ...
    if (isGroup) {
      ...
      const groupRoomId = roomId || name;
      
      const room = await db.room.create({ 
        id: groupRoomId,
        isGroup: true,
        name: name,
        photoId: parseInt(groupRoomId) || 1,
        presentUsers: []
      });
      ...
      if (userIds.length > 0) {
        ...
        await db.roomuser.bulkCreate(memberEntries);
      }
...
exports.getRoomParticipants = async (req, res) => {
  ...
  try {
    console.log('getRoomParticipants called with params:', req.params);
    let { roomId, name } = req.params;
    const userId = req.user.id;
    const isGroup = req.query.isGroup === 'true';

    // For private chat, generate room ID from sorted user IDs
    if (!isGroup && name && !isNaN(name)) {
      const otherUserId = parseInt(name);
    ...
    // 2. If room doesn't exist, create it
    if (!room) {
      ...
      // Add members based on room type
      if (isGroup) {
        // For group chat, add current user
        await db.roomuser.create({
          roomId,
          userId,
          createdAt: new Date(),
        ...
        // For private chat, add both users
...
//         // Create new private chat
//         const room = await db.room.create({ 
//           id: `${user1}-${user2}`, // Or fetch user names
//           isGroup: false,
...
// //   const transaction = await db.sequelize.transaction();
// //   try {
// //     console.log('getRoomParticipants called with params:', req.params, 'and user:', req.user);
...
// // REPLACE the getRoomParticipants function in roomController.js
// exports.getRoomParticipants = async (req, res) => {
//   const transaction = await db.sequelize.transaction();
//   try {
//     console.log('getRoomParticipants called with params:', req.params, 'and user:', req.user);
...
//     // For private chat, generate room ID from sorted user IDs
//     if (!isGroup && roomId && roomId.includes('-')) {
//       // roomId is already in format "userId1-userId2"
//       // Just use it as is
//     } else if (!isGroup && name) {
//       // Create room ID from user IDs
//       const otherUserId = parseInt(name);
...
//         name: name || roomId,
//         isGroup: isGroup || false,
//         photoId: isGroup ? parseInt(roomId) || 1 : 1, // For private chats, use default photoId
//         presentUsers: []
//       }, { transaction });

//       // Add members
//       if (isGroup) {
//         // For group chat, we need to get all members from somewhere
